...
const AdminOrders = () => {
  ...
  const columns: ResponsiveColumn<OrderItem>[] = [
    { key: "name", label: "Name", mobile: true, render: (o) => <span className="text-sm text-brand-grey900 font-medium truncate">{o.name}</span> },
    { key: "asset", label: "Asset", mobile: true, render: (o) => <span className="text-sm text-brand-grey900">{o.asset}</span> },
    { key: "type", label: "Type", render: (o) => <span className="text-xs text-brand-grey500">{o.type}</span> },
    { key: "amount", label: "Amount", mobile: true, render: (o) => <span className="text-sm text-brand-grey900">{o.amount}</span> },
    { key: "wallet", label: "Wallet Address", render: (o) => o.walletAddress !== "—" ? (
      <div className="flex items-center gap-1"><span className="text-xs text-brand-grey500 font-mono">{o.walletAddress}</span><CopyButton text={o.walletAddress} label="Wallet" /></div>
    ) : <span className="text-xs text-brand-grey500">—</span> },
    { key: "confirms", label: "Confirms", render: (o) => <span className="text-xs text-brand-grey500">{o.confirmations > 0 ? `${o.confirmations} ✓` : "—"}</span> },
    { key: "txId", label: "Trans ID", render: (o) => (
      <div className="flex items-center gap-1"><span className="text-xs text-brand-grey500 font-mono">{o.txId.slice(0, 10)}...</span><CopyButton text={o.txId} label="Trans ID" /></div>
    )},
    { key: "payoutRef", label: "Payout Ref", render: (o) => <span className="text-xs text-brand-grey500">{o.payoutRef}</span> },
    { key: "date", label: "Date", render: (o) => <span className="text-xs text-brand-grey500 whitespace-nowrap">{o.date}</span> },
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 mb-6">
        <div className="bg-brand-surface border border-brand-grey100 rounded-xl p-5">
          <p className="text-sm text-brand-grey500 mb-1">Total Order</p>
          <p className="text-2xl font-gasoek font-bold text-brand-grey900">{ordersData.totalOrder}</p>
          <p className="text-xs text-brand-grey500 mb-3">{ordersData.totalOrderBtc}</p>
          ...
          <div className="flex gap-3 text-[10px] text-brand-grey500">
            {ordersData.orderDistribution.map(d => <span key={d.label}>{d.label}</span>)}
          </div>
        </div>

        <div className="bg-brand-surface border border-brand-grey100 rounded-xl p-5">
          <p className="text-sm text-brand-grey500 mb-1">Total payout</p>
          <p className="text-2xl font-gasoek font-bold text-brand-grey900">{ordersData.totalPayout}</p>
          <p className="text-xs text-brand-grey500">{ordersData.totalPayoutNgn}</p>
          ...
          <div className="mt-3 bg-brand-danger/10 rounded-lg p-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <AlertCircle className="w-4 h-4 text-brand-danger" />
              <div>
                <p className="text-xs font-medium text-brand-grey900">Some payouts failed</p>
                <p className="text-[10px] text-brand-grey500">You have {statusCounts.FAILED} orders with failed payouts.</p>
              </div>
            </div>
            <button onClick={() => { setStatusFilter("FAILED"); setPage(1); }} className="h-7 px-3 bg-brand-danger text-white hover:bg-brand-danger/90 rounded-lg text-xs font-medium">Review</button>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-3">
        ...
        <div className="flex items-center gap-3 text-xs text-brand-grey500 flex-wrap">
          <div className="relative">
            ...
            <input value={searchQuery} onChange={e => { setSearchQuery(e.target.value); setPage(1); }}
              ...
              className="h-8 w-52 bg-brand-surface border border-brand-grey100 rounded-lg pl-8 pr-3 text-xs text-brand-grey900 placeholder:text-brand-grey500 outline-none" />
          </div>
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
        {(["All", "COMPLETED", "PENDING", "FAILED"] as const).map(s => (
          <button key={s} onClick={() => { setStatusFilter(s); setPage(1); }}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap ${statusFilter === s ? "bg-brand-blue500 text-white" : "bg-brand-grey100 text-brand-grey500 hover:bg-brand-grey50 hover:text-brand-blue500"}`}>
